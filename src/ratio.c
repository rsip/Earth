/*
NAME:		ratio.c 
DESCRIPTION: 	Divide values in image 1 by values in image 2
AUTHOR:	 	Will Grey
VERSION:	2015-11-09	
...
int ratio(int argc, char **argv)
{
 ...
 if (hdrData.dataType == 1) ratioByte(fin1,fin2,fout);
 if (hdrData.dataType == 2) ratioShort(fin1,fin2,fout);
 if (hdrData.dataType == 3) ratioLong(fin1,fin2,fout);
 if (hdrData.dataType == 4) ratioFloat(fin1,fin2,fout);
 if (hdrData.dataType == 8) ratioDouble(fin1,fin2,fout);

/*
NAME:		min.c 
DESCRIPTION: 	Calculate min of seqence of input images
AUTHOR:	 	Will Grey
VERSION:	2015-12-23	
...
int minImg(int argc, char **argv){
 ...
 if (hdrData.dataType == 1) minImgByte(fin,fout, &hdrData, nFiles);
 if (hdrData.dataType == 2) minImgShort(fin,fout, &hdrData,nFiles);
 if (hdrData.dataType == 3) minImgLong(fin,fout, &hdrData, nFiles);
 if (hdrData.dataType == 4) minImgFloat(fin,fout, &hdrData,nFiles); 
 if (hdrData.dataType == 8) minImgDouble(fin,fout, &hdrData, nFiles); 
...
int minImgByte(FILE **fin, FILE *fout, metaData *hdrData, int nFiles){
  ...
  for(x = 0; x < PIXELS; x++){ 
   ...
   for(file = 0; file < nFiles; file++) 
    if ((hdrData->ignoreValue == 0) || *(inImg[file] + x) != hdrData->nullValue)
     m = MINIMUM(m,(double)*(inImg[file] + x));
...
int minImgShort(FILE **fin, FILE *fout, metaData *hdrData, int nFiles){
...
int minImgLong(FILE **fin, FILE *fout, metaData *hdrData, int nFiles){
...
int minImgFloat(FILE **fin, FILE *fout, metaData *hdrData, int nFiles){
...
int minImgDouble(FILE **fin, FILE *fout, metaData *hdrData, int nFiles){

...
double simplexFit (double **x, int n, float *dist, float *sv, int num, int flag){
 ...
 for (i=0;i<n+1;i++){
  fx[i] = merit_function(x[i],dist,sv,num,flag);
  xpos[i]=i; 
 }
 ...
 fr=merit_function(xr,dist,sv,num,flag);
 ...
 if (fx[0] <= fr && fr < fx[n-1]){
  ...
  fe=merit_function(xe,dist,sv,num,flag); 
  ...
  fc=merit_function(xc,dist,sv,num,flag);
